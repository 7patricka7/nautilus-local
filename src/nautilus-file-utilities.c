                                   ...
                                   const gchar *new_display_name)
{
    ...
    else
    {
        ...
        if (old_recent)
        {
            gboolean recreate_recent_item = FALSE;
            gboolean recreated = FALSE;

            if (new_display_name != NULL && old_display_name == NULL)
            {
                recreate_recent_item = TRUE;
            }
            else
            {
                gboolean name_changed;
                gboolean was_same_name_in_recent;
                const gchar *old_recent_name;

                old_recent_name = gtk_recent_info_get_display_name (old_recent);

                name_changed = g_strcmp0 (old_display_name, new_display_name) != 0;
                was_same_name_in_recent = g_strcmp0 (old_display_name, old_recent_name) == 0;

                recreate_recent_item = (name_changed && was_same_name_in_recent);
            }

            if (recreate_recent_item)
            {
