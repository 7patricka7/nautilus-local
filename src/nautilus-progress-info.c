            ...
            const char           *short_status)
{
    ...
    real_short_status = short_status == NULL ? status : short_status;

    gboolean should_update = g_set_str (&info->short_status, real_short_status);

    if (g_set_str (&info->status, status) || should_update)
    {
